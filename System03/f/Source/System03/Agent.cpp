...
void AAgent::BeginPlay()
{
	...
	Velocity = FVector(FMath::FRand()*2.0f-1.0f,FMath::FRand()*2.0f-1.0f,FMath::FRand()*2.0f-1.0f)*5.0f;
	MaxVelocity *= 2;
	 MaxForce *= 1.5;
	...
	if (AgentType == 1){
		...
		AmountMovementNoise = FMath::FRand()* 4.0f + 1.0f;
	} else {Angle = 0;}
...
{	
	...
	Limit(Location,FVector(50.0f,50.0f,0.0f),FVector(ScenarioSize-50.0f,ScenarioSize-50.0f,0.0f));
	} else {
		if (Location.X<= 50.0f){ Position.X = ScenarioSize-51.0f;}
		if (Location.X>= ScenarioSize-50.0f){ Position.X = 49.0f;}
		if (Location.Y<= 50.0f){ Position.Y = ScenarioSize-51.0f;}
		if (Location.Y>= ScenarioSize-50.0f){ Position.Y = 49.0f;}
		if (Location.Z<= 50.0f){ Position.Z = ScenarioSize/2.0f-51.0f;}
		if (Location.Z>= ScenarioSize/2.0f-50.0f){ Position.Z = 49.0f;}
...
void AAgent::AddRandomMovement()
{
	...
	Noise = FMath::PerlinNoise3D(Position/20)*AmountMovementNoise*2.50f;  	
