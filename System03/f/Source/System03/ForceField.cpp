...
AForceField::AForceField()
{
    ...
    for (int i = 0; i < cols; i++) {
        ...
        for (int j = 0; j < cols; j++) { 
			...
			ArrowDireection = FVector((float)(j-cols/2)*1.0f,-(float)(i-cols/2)*1.0f,0.0f);
			ArrowDireection += FVector(-(float)(i-cols/2)*0.7f,-(float)(j-cols/2)*0.7,0.0f);
			//ArrowDireection.Normalize();
			field.Add (ArrowDireection);
