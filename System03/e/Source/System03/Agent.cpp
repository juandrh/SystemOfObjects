...
{	
	...
	Limit(Location,FVector(50.0f,50.0f,50.0f),FVector(ScenarioSize-50.0f,ScenarioSize-50.0f,ScenarioSize/2.0f-50.0f));
	} else {
...
void AAgent::RotateBody()    
{
	...
	if (AgentType == 1){ 
		// MeshChildren:   1 -> Head 
		Velocity2D =Velocity * FVector(1.0f,1.0f,1.0f);		
...
void AAgent::Boundaries()
{
	if (!isCyclicalScenario){
		...
		} 	else if (Position.Z <= 50.f || Position.Z >= ScenarioSize/2.0f -50.0f ) {
